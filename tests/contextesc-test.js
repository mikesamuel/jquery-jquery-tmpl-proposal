...
function rewrittenSource( input ) {
	...
	for ( var k in input ) {
		if ( Object.hasOwnProperty.call( input, k ) ) {
			inputParseTrees[ k ] = parseTemplate(
					input[ k ], DEFAULT_BLOCK_DIRECTIVES );
		}
	}
	...
	assertContextualRewriting(
			{
				"bar": join(
						"<a",
						"{{if $url}}",
							" href='${$.encode.escapeHtmlAttribute(",
							"$.encode.filterNormalizeUri(url))}'>",
						"{{else $name}}",
	...
	assertContextualRewriting(
			{
				"bar": join(
						"<script>\n",
							...
							"boo(/${$.encode.escapeJsRegex(d)}/.test(s)",
							" ? 1 / ${$.encode.escapeJsValue(e)}",
	...
	assertContextualRewriting(
			{
				"bar": join(
						...
						"<a href='${$.encode.escapeHtmlAttribute(",
							"$.encode.filterNormalizeUri(url))}'",
						" style='background:url(",
							"${$.encode.escapeHtmlAttribute($.encode.filterNormalizeUri(",
								"bgimage))})'>",
						"Hi</a>",
						"<a href='#${$.encode.escapeHtmlAttribute(anchor)}'",
						// escapeUri for substitutions into queries.
						" style='background:url(",
							"&apos;/pic?q=${$.encode.escapeUri(file)}&apos;)'>",
							"Hi",
						"</a>",
						"<style>",
							"body { background-image: url(",
								"\"${$.encode.filterNormalizeUri(bg)}\"); }",
							// and normalizeUri without the filter in the path.
							"table { border-image: url(",
								"\"borders/${$.encode.normalizeUri(brdr)}\"); }",
						"</style>" )
			},
			{
				"bar": join(
	...
	assertContextualRewriting(
			{
				"foo": join(
						...
						"${$.encode.escapeHtmlAttributeNospace(",
							"$.encode.escapeJsValue(msg))})>",
						"Launch</button>" )
			},
			{
	...
	assertContextualRewriting(
			{
				"iconTemplate": join(
						...
						" src=",
						"{{if iconPath}}",
							"\"${$.encode.escapeHtmlAttribute(",
								"$.encode.filterNormalizeUri(iconPath))}\"",
						"{{else}}",
							"\"images/cleardot.gif\"",
						"{{/if}}",
						"{{if title}}",
	...
	assertContextualRewriting(
			{
				"myTmpl": join(
						...
						"{{wrap \"listWrapper__C8208\"}}",
							...
							"<li title=${$.encode.escapeHtmlAttributeNospace(t)}>",
							"${$.encode.escapeHtml(b)}</li>",
						"{{/wrap}}" ),
				"listWrapper": join(
						"{{each $item.html(\"div\")}}",
