	...
	try {
		...
		for ( var i = 0, n = outputTemplateNames.length; i < n; ++i ) {
			...
			sanitizedTemplateText = renderParseTree(
					$.templates[ templateName ].tmpl.parseTree,
					DEFAULT_BLOCK_DIRECTIVES );
			$( "<h3/>" ).text( templateName ).appendTo( outputHtml );
			$( "<pre/>" )
					.html( templateTextToDisplayableHtml( sanitizedTemplateText ) )
					.appendTo( outputHtml );
			...
			if ( inDom.length && "SCRIPT" !== inDom[ 0 ].tagName ) {
				...
				inDom.detach();
				inDom = $( "<script>",
									 { id: templateName, type: "text/x-jquery-tmpl" } )
...
ESC_MODE_HELP[ escapeJsString.name ] = "foo \u2192 fo\\o";
ESC_MODE_HELP[ escapeJsValue.name ] = "foo \u2192 \"fo\\o\"";
ESC_MODE_HELP[ escapeJsRegex.name ] = "a+b \u2192 a+b";
ESC_MODE_HELP[ escapeCssString.name ] = "foo \u2192 fo\\o for CSS";
				...
				for ( var i = operators.length; --i >= 0; ) {
					...
					if ( sanitizer ) {
						operators[ i ] = "<ins class=\"sanitizer\""
								+ " title=\""
								+ escapeHtml( ESC_MODE_HELP[ sanitizer[ 1 ] ] || "" )
								+ "\">" + operators[ i ] + "</ins>";
					}
				}
